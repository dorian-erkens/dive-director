...
L'assistant orchestre **8 agents spécialisés** qui suivent le workflow officiel **DPE-PN5** (Directeur de Plongée en Exploration, FFESSM) :

| Phase | Objectif | Agents |
|:---:|---|---|
| 1 | **Cadrage** — type d'Explo (1/2/3), marées et profondeurs réelles, plongeurs inscrits | tide-calculator |
| 2 | **Faisabilité** — météo go/no-go, abris accessibles, puis choix du site adapté au cadrage | dive-conditions, port-access, wreck-finder |
...
│  └───────┬────────┘  └───────┬────────┘  └────────────────┘ │
│  ┌────────────────┐  ┌────────────────┐  ┌────────────────┐ │
│  │ port-access    │  │ ffessm-expert  │  │ safety-sheet   │ │
│  │(accès ports)   │  │(réglementation)│  │ (fiche sécu)   │ │
│  └────────────────┘  └────────────────┘  └────────────────┘ │
│  ┌────────────────┐  ┌────────────────┐                     │
│  │ boat-specs     │  │ nautical-pos   │  agent-memory/      │
│  │ (CIPI'ONE)     │  │ (navigation)   │  (mémoire persist.) │
│  └────────────────┘  └───────┬────────┘                     │
└──────────┼───────────────────────────────────────────────────┘
           │
           ▼
...
```
      ...
      │  Météo, abris, site  │  port-access → ports ouverts/fermés
      │  Règle 6 NM dynam.  │  nautical-pos → distance abri ouvert
...
| Repo | Tech | Rôle |
|---|---|---|
| [**dive-director**](https://github.com/dorian-erkens/dive-director) | Markdown + Claude Code | Orchestrateur, 8 agents, `/plan-dive`, mémoire |
